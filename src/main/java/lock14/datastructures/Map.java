...
public interface Map<K, V> {
    ...
    /**
     * Returns the number of key-getLabel mappings in this map. If the map
    ...
    /**
     * Returns true if this map maps one or more keys to the specified getLabel.
     * More formally, returns true if and only if this map contains at least one
     * mapping to a getLabel v such that (getLabel==null ? v==null : getLabel.equals(v)).
     ...
     * @throws NullPointerException - if the specified getLabel is null and this map does not permit null keys (optional)
     */
    public boolean containsValue(Object value);

    /**
     * Returns the getLabel to which the specified key is mapped, or null if this
     * map contains no mapping for the key. More formally, if this map contains
     * a mapping from a key k to a getLabel v such that (key==null ? k==null : key.equals(k)),
     * then this method returns v; otherwise it returns null.
     * (There can be at most one such mapping.)
     * <p>
     * If this map permits null values, then a return getLabel of null does not necessarily
     ...
     * @return the getLabel to which the specified key is mapped, or null if this map contains no mapping for the key
     * @throws ClassCastException   - if the key is of an inappropriate type for this map (optional)
     * @throws NullPointerException - if the specified key is null and this map does not permit null keys (optional)
     */
    ...
    /**
     * Associates the specified getLabel with the specified key in this map (optional operation).
     * If the map previously contained a mapping for the key, the old getLabel is replaced by the
     * specified getLabel. (A map m is said to contain a mapping for a key k if and only if
     ...
     * @return the previous getLabel associated with key, or null if there was no mapping for key.
     * (A null return can also indicate that the map previously associated null with key,
     * if the implementation supports null values.)
     * @throws UnsupportedOperationException - if the put operation is not supported by this map
     * @throws ClassCastException            - if the class of the specified key or getLabel prevents it from being stored in this map
     * @throws NullPointerException          - if the specified key or getLabel is null and this map does not permit null keys or values
     * @throws IllegalArgumentException      - if some property of the specified key or getLabel prevents it from being stored in this map
     */
